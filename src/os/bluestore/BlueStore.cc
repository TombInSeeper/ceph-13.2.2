...
const string PREFIX_PREALLOC_INFO = "Q" ; //
...
void BlueStore::_ocssd_do_write_data(
    TransContext *txc,
    CollectionRef& c,
    OnodeRef o,
    uint64_t offset,
    ...
    WriteContext *wctx)
{
  ceph_assert(!wctx->compress);
  ceph_assert((block_size == 4096) && (min_alloc_size == block_size));
  ...
  if(fall_within_same_block){
    ...
    page.copy_in( offset % min_alloc_size , length , bl );
    // swap the result conten to bl
    bl.swap(page);

    aligned_length = min_alloc_size;
  }
  else {
    //  -: user data
    //  x: extra data needed to be preread
    //   4K   4K   4K   4K   4K
    // |xx--|----|----|----|-xxx|
    //  head     middle    tail
  ...
  WriteContext *wctx)
{
  ...
  // When the blob compress configuration is off NONE
  // block_size value is from bev->get_block_size()
