  ...
  *_dout << dendl;
  ...
  if(s2 == nullptr){
    dout(10) << " OCDevice::write " << "I'm now tring to append data to ONE superblock" <<
      ...
      " obj_logical_id = " << s1->bdev_lba_offset / (s1->size) << std::hex <<
      " obj_bdev_lba_off = " << s1->bdev_lba_offset  <<
      " obj_nvm_off = " << s1->nvm_obj_off  <<
      " blue_offset = " << ( off ) <<
      " data length  = " << bl.length() << std::dec
      << dendl;

    dout(10) << std::hex <<
             " off % s1->size = 0x" << off % s1->size <<
             " s1->nvm_obj_off = 0x" << s1->nvm_obj_off
             << std::dec
             << dendl;
...
void SegmentManager::_segment_discard(uint64_t offset, uint32_t length, interval_set<uint64_t> *free) {
  ...
  else
  {
    ...
    tail_offset = p2align(end, segment_size);
