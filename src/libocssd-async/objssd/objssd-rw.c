...
int __obj_rw(struct nvm_dev *dev, io_u *io, int op_type)
{
    ...
    if(first_step_size == 24)  first_step_size = 0;
    else if(first_step_size > data_size) first_step_size = data_size;
    int i = 0;
    for(i = 0; (i + 8) < first_step_size; i = i + 8){
        init_queue_node(node_t, io->obj_id, io->obj_off + i, (void *)(io->data + i * 4096), 8, io->io_req_tag, dev, op_type, complete_bit + 5);
    ...
    if(i != first_step_size){
        init_queue_node(node_t, io->obj_id, io->obj_off + i, io->data + i * 4096, first_step_size - i, io->io_req_tag, dev, op_type, &(complete_bit[cmd_num ++]));
        obj_addr_to_oc_addr(dev, &addr_t, io->obj_id, io->obj_off + i);
        int lun_index = addr_t.g.ch * geo->nluns + addr_t.g.lun;
        while(1){  
            if(EnQueue(queue_table[lun_index], node_t) == 0) 
