...
static ssize_t _vblk_write(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	...
	struct hal_timer t;
	hal_timer_start(&t);		// Allocate write buffer
	...
	hal_timer_stop(&t);
	hal_timer_pr(&t, "nvm_buf_fill");

	hal_timer_start(&t);
	res = nvm_vblk_write(vblk, buf, nbytes);// Write buffer to device
	hal_timer_stop(&t);
	hal_timer_bw_pr(&t, "nvm_vblk_write", nbytes);
...
static ssize_t _vblk_read(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	...
	struct hal_timer t;
	hal_timer_start(&t);		// Allocate read buffer
	buf = nvm_buf_alloc(geo, nbytes);
	if (!buf) {
	...
	hal_timer_stop(&t);
	hal_timer_bw_pr(&t, "nvm_vblk_read", nbytes);
...
static ssize_t _vblk_pread(struct nvm_cli *cli, struct nvm_vblk *vblk)
{
	...
	hal_timer_start(&t);		// Read from device into buffer
	res = nvm_vblk_pread(vblk, buf, nbytes, offset);
	hal_timer_stop(&t);
	hal_timer_bw_pr(&t, "nvm_vblk_pread", nbytes);
...
static ssize_t _vblk_pad(struct nvm_cli *NVM_UNUSED(cli), struct nvm_vblk *vblk)
{
	...
	hal_timer_stop(&t);
	hal_timer_bw_pr(&t, "nvm_vblk_pad", nvm_vblk_get_nbytes(vblk));
